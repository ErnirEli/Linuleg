...
def equal(A, B):
    ...
    return not any([not (A[k] == B[k]) for k in A.f])
...
def add(A, B):
    ...
    # for k in A.D[0]:
    #     for n in A.D[1]:
    #         print('A + B: ', A[k, n] + B[k, n])

    return Mat(A.D, {(k,n): A[k, n] + B[k, n] for k in A.D[0] for n in A.D[1]})
        

    # return Mat(A.D, {tuple(k): A[str(k)[1:-1]] + B[str(k)[1:-1]] for k in A.D})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {key: M[key] * x for key in M.f})
...
def transpose(M):
    ...
    # k = Mat(M.D, {(key[1], key[0]): M[key[0], key[1]] for key in M.f})
    # print('#####################################################################################')

    # print(M.f)
    # print(M)
    # for key in M.f:
    #     a = {(key[1], key[0]): M[key[0], key[1]] for key in M.f}

    # print(a)
    # print(M.D)

    # print(Mat((M.D[1], M.D[0]), {(key[1], key[0]): M[key[0], key[1]] for key in M.f}))

    # print('#####################################################################################')

    return Mat((M.D[1], M.D[0]), {(key[1], key[0]): M[key[0], key[1]] for key in M.f})


def vector_matrix_mul(v: Vec, M):
    """
    ...
    return Vec(M.D[1], dic)
    # print(Vec(M.D[1], {k: a for k in M.D[1] a = M[n, k] * v[n] for n in M.D[0]}))
